...
#==============================================================================#
                    ...
                    nadya.sendMessage(to, "เพิ่มเพื่อนอัตโนมัติเปิดทำงานแล้ว")
                elif text.lower() == 'แอดออโต้ ปิด ':
                    settings["autoAdd"] = False
                    nadya.sendMessage(to, "เพิ่มเพื่อนอัตโนมัติปิดทำงานแล้ว")
                elif text.lower() == 'เข้าออโต้ เปิด':
                    settings["autoJoin"] = True
                    nadya.sendMessage(to, "เปิดการทำงานเข้าห้องอัตโนมัติแล้ว")
                elif text.lower() == 'เข้าออโต้ ปิด':
                    settings["autoJoin"] = False
                    nadya.sendMessage(to, "ปิดการทำงานเข้าห้องอัตโนมัติแล้ว")
                elif text.lower() == 'ป้องกันแชท เปิด':
                    settings["autoLeave"] = True
                    nadya.sendMessage(to, "เปิดฟังชั่นป้องกัน แชทรวมทำงานแล้ว")
                elif text.lower() == 'ป้องกันแชท ปิด':
                    settings["autoLeave"] = False
                    nadya.sendMessage(to, "ปิดฟังชั่นป้องกัน แชทรวมทำงานแล้ว")
                elif text.lower() == 'อ่าน เปิด':
                    settings["autoRead"] = True
                    nadya.sendMessage(to, "เปิดการอ่านข้อความอัตโนมัติแล้ว")
                elif text.lower() == 'อ่าน ปิด':
                    settings["autoRead"] = False
                    nadya.sendMessage(to, "ปิดการอ่านข้อความอัตโนมัติแล้ว")
                elif text.lower() == 'สติกเกอ เปิด':
                    settings["checkSticker"] = True
                    nadya.sendMessage(to, "เปิดการทำงานป้องกันฟัดสติกเกอร์แล้ว")
                elif text.lower() == 'สติกเกอ ปิด':
                    settings["checkSticker"] = False
                    nadya.sendMessage(to, "ปิดการทำงานป้องกันฟัดสติกเกอร์แล้ว")
                elif text.lower() == 'แทคแตะ เปิด':
                    settings["datectMention"] = True
                    nadya.sendMessage(to, "เปิดการทำงานแตะคนแท็ครายชื่อแล้ว")
                elif text.lower() == 'แทคแตะ ปิด':
                    settings["datectMention"] = False
                    nadya.sendMessage(to, "ปิดการทำงานแตะคนแท็ครายชื่อแล้ว")
...
#==============================================================================#
                ...
                elif msg.text.lower().startswith("เช็คคท "):
                ...
                elif msg.text.lower().startswith("เช็คไอดี "):
                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                        names = re.findall(r'@(\w+)', text)
                        mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                        mentionees = mention['MENTIONEES']
                ...
                elif msg.text.lower().startswith("เช็คชื่อ "):
                    if 'MENTION' in msg.contentMetadata.keys()!= None:
                        names = re.findall(r'@(\w+)', text)
                        mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                        mentionees = mention['MENTIONEES']
...
#==============================================================================#
                ...
                elif text.lower() == 'ลิ้งกลุ่ม เปิด':
                    if msg.toType == 2:
                        group = nadya.getGroup(to)
                        if group.preventedJoinByTicket == False:
                ...
                elif text.lower() == 'รายละเอียดกลุ่ม':
                    group = nadya.getGroup(to)
                    try:
                    ...
                    ret_ += "\n╠ ผู้ดูแล : {}".format(str(gCreator))
                    ret_ += "\n╠ จำนวนสมาชิค : {}".format(str(len(group.members)))
                    ret_ += "\n╠ จำนวนสมาชิคค้างเชิญ : {}".format(gPending)
                    ret_ += "\n╠ Group Qr : {}".format(gQr)
                    ret_ += "\n╠ Group Ticket : {}".format(gTicket)
                    ret_ += "\n╚══[ Finish ]"
                    nadya.sendMessage(to, str(ret_))
                    nadya.sendImageWithURL(to, path)
                elif text.lower() == 'เช็คกลุ่มสมช':
                    if msg.toType == 2:
                        group = nadya.getGroup(to)
                        ret_ = "╔══[ Member List ]"
                        no = 0 + 1
                        ...
                        nadya.sendMessage(to, str(ret_))
                elif text.lower() == 'เช็ครายชื่อกลุ่ม':
                        groups = nadya.groups
                        ret_ = "╔══[ Group List ]"
                        no = 0 + 1
                        for gid in groups:
